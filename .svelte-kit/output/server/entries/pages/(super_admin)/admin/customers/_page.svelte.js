...
const CustomersComp = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { data } = $$props;
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `<div>${data?.customers?.items ? `
      <div class="grid relative md:grid-cols-3 gap-6">${each(data?.customers?.items, (item) => {
    ...
    ${data?.customers?.items.length == 0 ? `<p>No Products Found.</p>` : ``}</div>`;
});
...
<div>${data?.error ? `${validate_component(ErrorComp, "ErrorComp").$$render($$result, { error: data?.error }, {}, {})}` : ``}
  ${data?.customers ? `
    ${validate_component(CustomersComp, "CustomersComp").$$render($$result, { data }, {}, {})}` : ``}</div>`;
});
export {
  Page as default
};
